...
class SouFangOldHouseProcessor extends BasePageProcessor implements PageProcessor {
    ...
    private static void crawlPageInfo(Page page) {
        ...
        String baseInfo = page.getHtml().xpath(Pattern.X_M_BASE_INFO).get()
        page.putField("baseInfo", baseInfo)
        String courtIntroduce = page.getHtml().xpath(Pattern.X_M_COURT_INTRODUCE).get()
        page.putField("courtIntroduce", courtIntroduce)
        String matingInfo = page.getHtml().xpath(Pattern.X_M_MATING_INFO).get()
        page.putField("matingInfo", matingInfo)
        String surrounding = page.getHtml().xpath(Pattern.X_M_SURROUNDING).get()
        page.putField("surrounding", surrounding)
        String traffic = page.getHtml().xpath(Pattern.X_M_TRAFFIC).get()
